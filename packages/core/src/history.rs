...
mod tests {
    ...
    impl HistoryStore for InMemoryHistoryStore {
        ...
        fn delete(&mut self, id: &str) -> crate::Result<()> {
            if let Some(pos) = self.entries.iter().position(|e| e.id == id) {
                ...
                Err(crate::ShardDenError::NotFound(format!(
    ...
    fn test_history_store_list_by_tool() {
        ...
        store
            .add(HistoryEntry::new("json-extractor", "{}", "result1", false))
            .unwrap();
        store
            .add(HistoryEntry::new("json-extractor", "{}", "result2", false))
            .unwrap();
        store
            .add(HistoryEntry::new("other-tool", "{}", "result3", false))
            .unwrap();
    ...
    fn test_history_store_search() {
        ...
        store
            .add(HistoryEntry::new("tool", "hello world", "result1", false))
            .unwrap();
        store
            .add(HistoryEntry::new("tool", "foo bar", "result2", false))
            .unwrap();
        store
            .add(HistoryEntry::new("tool", "hello again", "result3", false))
            .unwrap();
    ...
    fn test_history_store_clear() {
        ...
        store
            .add(HistoryEntry::new("tool", "input1", "output1", false))
            .unwrap();
        store
            .add(HistoryEntry::new("tool", "input2", "output2", false))
            .unwrap();
